                ...
                <div className="space-y-12">
                    {experiences.map((exp, idx) => (
                        <div key={idx} className="pb-10 animate-fade-in"
                            ...
                            {/* content */}
                            <div className="flex flex-col md:flex-row  items-start gap-4 p-10 rounded-2xl  transition-all duration-500">
                                <span className="text-sm text-muted-foreground font-mono shrink-0 md:items-start">
                                    {exp.period}
                                </span>
                                ...
                                <div className="flex flex-col items-center w-full">
                                    ...
                                    <p className="text-xs md:text-sm text-muted-foreground mt-4 text-center w-72">{exp.description}</p>

                                    <div className="flex flex-wrap gap-2 pt-4 justify-center">
                                        {exp.technology.map((tech, indxtech) => (
                                            <span 
                                                key={indxtech} 
                                                className="px-3 py-1  text-xs font-mono rounded-lg border border-white/10 text-muted-foreground"
                                            >
                                                {tech}
                                            </span>
                                        ))}
